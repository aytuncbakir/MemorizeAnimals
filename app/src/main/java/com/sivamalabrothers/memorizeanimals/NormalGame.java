...
public class NormalGame extends AppCompatActivity implements TextToSpeech.OnInitListener,
        View.OnClickListener,Animation.AnimationListener{

    TextToSpeech textToSpeech;
    ImageView image1, image2, image3, image4, image5,image6;
    ImageView image11, image12, image7, image8, image9,image10;

    ImageView imageBir, imageIki, imageUc;
    ImageView imageDort, imageBes, imageAlti;
    ...
    String text = "";
    static int normalLevel = 0;
    private ProgressBar progressBar;
    private TextView textView;
    private Handler handler = new Handler();
    private int progressStatus = 100;
    TextView skor,levelGoster;
    static int normalScore = 0;
    AlphaAnimation fadeIn, fadeOut;
    static int normalDurum = 0;
    boolean suspended = false;
    SharePreferences sharePreferences;
    String sonSkor = "normalSkor";
    String maksSkor = "normalMaxSkor";
    String sonLevel = "normalLevel";
    MediaPlayer ses;
    ...
    private static final int CUSTOM_DIALOG_ID_NORMAL_BACK = 0;
    private static final int CUSTOM_DIALOG_ID_NORMAL_NEXT = 1;
...
*/
    ...
    private void init(){
        ...
        normalLevel = sharePreferences.getInt(context,sonLevel) ;
        //Toast.makeText(context," İnit Level "+normalLevel,Toast.LENGTH_LONG).show();

        maxSkor = sharePreferences.getInt(context,maksSkor);
        normalScore = sharePreferences.getInt(context,sonSkor);

        if(normalLevel == animals.size()){
            normalLevel = 0;
            normalScore = 0;
            sharePreferences.saveInt(context,maksSkor,maxSkor);
            sharePreferences.saveInt(context,sonSkor,0);
            sharePreferences.saveInt(context,sonLevel,normalLevel);
        }
        ...
        String formattedLevel = levelLanguage+ String.valueOf(sharePreferences.getInt(context, sonLevel) / 6 + 1);
    ...
    public void onAnimationEnd(Animation animType) {
        ...
        if(animType.equals(fadeIn) || animType.equals(fadeOut)) {
            ...
            normalScore += 1;
            skor.setText(String.valueOf(normalScore));
            if (normalDurum < puan) {
                skor.startAnimation(fadeIn);
                skor.startAnimation(fadeOut);
            } else {
    ...
    private void animalsKonumlaArrayList(){
        ...
        for(int i = 0; i<yerlesimBelirle.size(); i++){
            if(yerlesimBelirle.get(i) == 1)
                animalsKonumla.add(animals.get(normalLevel));
            else if(yerlesimBelirle.get(i) == 2)
                animalsKonumla.add(animals.get(normalLevel+1));
            else if(yerlesimBelirle.get(i) == 3)
                animalsKonumla.add(animals.get(normalLevel+2));
            else if(yerlesimBelirle.get(i) == 4)
                animalsKonumla.add(animals.get(normalLevel+3));
            else if(yerlesimBelirle.get(i) == 5)
                animalsKonumla.add(animals.get(normalLevel+4));
            else if(yerlesimBelirle.get(i) == 6)
                animalsKonumla.add(animals.get(normalLevel+5));
        }
    }
    ...
    private void eslendiMi(){
        ...
        if(oyunBittiMi == 6){
            ...
            */
            ...
            imageBir.setBackgroundResource(animals.get(normalLevel).getImage());
            ...
            textBir.setText(animals.get(normalLevel).getName());



            imageIki = new ImageView(context);
            imageIki.setLayoutParams(params1);
            imageIki.setBackgroundResource(animals.get(normalLevel+1).getImage());
            ...
            textIki.setText(animals.get(normalLevel+1).getName());
            ...
            imageUc.setBackgroundResource(animals.get(normalLevel+2).getImage());
            ...
            satir3.addView(imageBes);
            satir3.addView(textBes);
...
*/
            ...
            play_next.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    sharePreferences.saveInt(context,sonSkor,normalScore);
                    sharePreferences.saveInt(context,sonLevel,normalLevel);
                    tekrarBaslat();
                }
            });
    ...
    protected Dialog onCreateDialog(int id) {
        switch (id){
            case CUSTOM_DIALOG_ID_NORMAL_BACK:
                ...
                AnimalCustomAdapter girisMenuCustomAdapter =
                        new AnimalCustomAdapter(this,AnimalMenuItem.getAnimalMenuItems(normalLevel,0,
                                animals,2));
        ...
        recyclerView.setLayoutManager(gridLayoutManager);  */
            ...
            case CUSTOM_DIALOG_ID_NORMAL_NEXT:
                ...
                AnimalCustomAdapter girisMenuCustomAdapter1 =
                        new AnimalCustomAdapter(this,AnimalMenuItem.getAnimalMenuItems(normalLevel,1,
                                animals,2));
    ...
    private void tekrarBaslat(){
        Intent intent = new Intent(context, NormalGame.class);
        intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
        startActivity(intent);
    ...
    @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN)
    private void rotateImages(final ImageView image){

        rotate = ObjectAnimator.ofFloat(image, "rotationY", 0.0f, 180f);
